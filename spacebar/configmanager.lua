...
-- === Config module ===
log.i('Loading module: spacebar.configmanager')

local Config = {}

Config.types = { -- {{{
    -- validator & coerce mthods for each type found in spacebar config
    ['string'] = {
...
Config.schema = { -- {{{
    -- Set type for each spacebar config key
    paths = {
      yabai               = 'string'
    }
} -- }}}
...
function Config:validate(conf) -- {{{
    ...
    if isValid then
        ...
        log.e('Invalid spacebar config:\n', hs.inspect(err))
        hs.notify.new(nil, {
            title           = 'Invalid spacebar config!',
